  ...
  def next(self, split=True, iter=True):
    ...
    if dataset.size[0] > 0:
      iter["train"] = torchtext.data.BucketIterator(
        train_data, batch_size=self.config.batch_size, shuffle=True, sort_key=lambda x: len(x.text))
    if dataset.size[1] > 0:
      iter["val"] = torchtext.data.BucketIterator(
        val_data, batch_size=self.config.test_batch_size, sort_key=lambda x: len(x.text))
    if dataset.size[2] > 0:
      iter["test"] = torchtext.data.BucketIterator(
        test_data, batch_size=self.config.test_batch_size, sort_key=lambda x: len(x.text))
    return iter
